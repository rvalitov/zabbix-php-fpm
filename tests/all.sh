...
oneTimeSetUp() {
  #Install files:
  sudo cp "../zabbix/zabbix_php_fpm_discovery.sh" "/etc/zabbix"
  sudo cp "../zabbix/zabbix_php_fpm_status.sh" "/etc/zabbix"
  sudo cp "../zabbix/userparameter_php_fpm.conf" "$(find /etc/zabbix/ -name 'zabbix_agentd*.d' -type d | head -n1)"
